...
class Condition(object):
    ...
    def sql_condition(self, DataModelClass):
        condition = None
...
class BoolNot(object):
    ...
    def sql_condition(self, DataModelClass):
        return not_(self.condition.sql_condition(DataModelClass))
...
class BoolAnd(object):
    ...
    def sql_condition(self, DataModelClass):
        sql_conditions = [condition.sql_condition(DataModelClass) for condition in self.conditions]
...
def parse_search_expression(search_expression):
    return expression.parseString(search_expression)[0]


def run_search_expression(DataModelClass, query, parsed_expression):
    query = query.filter(parsed_expression.sql_condition(DataModelClass))
    return query
